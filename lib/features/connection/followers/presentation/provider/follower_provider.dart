...
final followerProvider = AsyncNotifierProvider<FollowerNotifier, List<User>>(
  () {
    return FollowerNotifier();
  },
);

class FollowerNotifier extends AsyncNotifier<List<User>> {
  ...
  Future<List<User>> build() async {
    return await _loadInitialFollowers();
  }

  // Load first page
  Future<List<User>> _loadInitialFollowers() async {
    try {
      final response = await _repository.getInitialFollowers();
  ...
  Future<void> loadMoreFollowers() async {
    ...
    try {
      ...
      final response = await _repository.getNextFollowers(
  ...
  Future<void> refreshFollowers() async {
    ...
    try {
      final users = await _loadInitialFollowers();
